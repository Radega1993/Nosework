import Link from "next/link";
import { useLocalizedLink } from "@/hooks/useLocalizedLink";

const linkClass =
    "text-white/80 hover:text-gold transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 focus-visible:ring-offset-navy-dark rounded";

export default function Footer() {
    const currentYear = new Date().getFullYear();
    const { localizedHref } = useLocalizedLink();

    const navLinks = {
        principal: [
            { name: "Inicio", path: "/" },
            { name: "Qué es", path: "/que-es-nosework-trial" },
            { name: "Reglamento", path: "/reglamento" },
            { name: "Cómo Empezar", path: "/como-empezar" },
        ],
        recursos: [
            { name: "Eventos", path: "/eventos" },
            { name: "Quiénes Somos", path: "/about" },
            { name: "Contacto", path: "/contact" },
        ],
    };

    const legalLinks = [
        { name: "Aviso legal", path: "/aviso-legal" },
        { name: "Política de privacidad", path: "/politica-privacidad" },
        { name: "Política de cookies", path: "/politica-cookies" },
    ];

    return (
        <footer className="bg-navy-dark text-white" role="contentinfo">
            <div className="container-redesign py-12">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {/* Brand */}
                    <div className="lg:col-span-2">
                        <h3 className="text-xl font-bold text-white mb-4">Nosework Trial Community</h3>
                        <p className="text-white/80 mb-4 max-w-md">
                            Promovemos el nosework deportivo como una modalidad inclusiva y accesible para todos los perros y sus guías.
                        </p>
                        <div className="flex space-x-4">
                            <a
                                href="#"
                                className={linkClass}
                                aria-label="Facebook"
                            >
                                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                                </svg>
                            </a>
                            <a
                                href="#"
                                className={linkClass}
                                aria-label="Instagram"
                            >
                                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                                </svg>
                            </a>
                        </div>
                    </div>

                    {/* Navegación */}
                    <div>
                        <h4 className="text-white font-semibold mb-4">Navegación</h4>
                        <ul className="space-y-2">
                            {navLinks.principal.map((link) => (
                                <li key={link.path}>
                                    <Link href={localizedHref(link.path)} className={linkClass}>
                                        {link.name}
                                    </Link>
                                </li>
                            ))}
                            {navLinks.recursos.map((link) => (
                                <li key={link.path}>
                                    <Link href={localizedHref(link.path)} className={linkClass}>
                                        {link.name}
                                    </Link>
                                </li>
                            ))}
                        </ul>
                    </div>

                    {/* Contacto */}
                    <div>
                        <h4 className="text-white font-semibold mb-4">Contacto</h4>
                        <p className="text-white/80 mb-3">
                            ¿Preguntas? Escríbenos.
                        </p>
                        <Link
                            href={localizedHref("/contact")}
                            className={linkClass}
                        >
                            Ir a Contacto
                        </Link>
                    </div>

                    {/* Legal (enlaces también en barra inferior) */}
                    <div>
                        <h4 className="text-white font-semibold mb-4">Legal</h4>
                        <ul className="space-y-2">
                            {legalLinks.map((link) => (
                                <li key={link.path}>
                                    <Link href={localizedHref(link.path)} className={linkClass}>
                                        {link.name}
                                    </Link>
                                </li>
                            ))}
                        </ul>
                    </div>
                </div>

                {/* Barra inferior: legal + copyright */}
                <div className="border-t border-white/20 mt-8 pt-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <nav className="flex flex-wrap gap-x-6 gap-y-1 justify-center sm:justify-start" aria-label="Enlaces legales">
                        {legalLinks.map((link) => (
                            <Link
                                key={link.path}
                                href={localizedHref(link.path)}
                                className={`text-sm text-white/80 hover:text-gold transition-colors duration-200 ${linkClass}`}
                            >
                                {link.name}
                            </Link>
                        ))}
                    </nav>
                    <p className="text-sm text-white/60 text-center sm:text-right">
                        &copy; {currentYear} Nosework Trial Community. Todos los derechos reservados.
                    </p>
                </div>
            </div>
        </footer>
    );
}
